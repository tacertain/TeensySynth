...
void MIDIController::queryUSBDeviceInfo() {
    ...
    uint16_t vid = midiDevice.idVendor();
    uint16_t pid = midiDevice.idProduct();
    ...
    const uint8_t* mfgStr = midiDevice.manufacturer();
    const uint8_t* prodStr = midiDevice.product();
    const uint8_t* serialStr = midiDevice.serialNumber();
